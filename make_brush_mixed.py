...
sofsetdict = {'10': 1.7, '12': 2.0, '14': 2.2, '16': 2.4, '18': 2.7, '18U': 2.5,
              '20': 2.9}
taildict = {'2': 5, '10': 29, '12': 35, '14': 41, '16': 47, '18': 53, '18U': 51,
              '20': 59}

nCx = '2'
lig_cx = LigandMolecule(fns=[f"minimize/molecules/C{nCx}T.itp",
                             f"minimize/molecules/C{nCx}T.gro"],
        head=2, tail=taildict[nCx], bind_group=[2])

nCy = '18U'
lig_cy = LigandMolecule(fns=[f"minimize/molecules/C{nCy}T.itp",
                             f"minimize/molecules/C{nCy}T.gro"],
        head=2, tail=taildict[nCy], bind_group=[2])
...
xtal_lx = 7.4 #lxdict[sys.argv[2]]
xtal_ly = xtal_lx
xtal_lz = 11*xlpar
box_params = {'sepz': 7.0}

apl = float('0.22') #Area per ligand in nm^2
llpar = math.sqrt(2*apl)
r0 = 0.20 # in nm
solvent_offset = sofsetdict[nCy] # in nm

#ligands = [lig_cx]
ligands = [lig_cx, lig_cy]
#ligand_pop_ratio = np.array([1])
ligand_pop_ratio = np.array([1,9])

mf = ligand_pop_ratio[0]/sum(ligand_pop_ratio)
tag = f"slvbrush_C{nCx}-C{nCy}-mf_{mf:.1f}-apl_{apl:.2f}_{solvent[0].name}"
dn = f"minimize/C{nCx}-C{nCy}-{solvent[0].name}/apl_{apl:.2f}/mf_{mf:.1f}"
...
if len(ligands) > 1:
    for each in ligands:
        pop_each = brush.molecule_pop[each.name]
        print(f"  #Ligand {each.name} per surface = ", pop_each/2)
        print(f"Ligand {each.name} per surface = ", pop_each/2,
              file=open(f"{dn}/info.txt", 'a'))
