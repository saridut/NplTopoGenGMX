...
class Brush(System):
    def __init__(self, name, fns_ff, is_slab, slab_pos):
        ...
        self.lig_lattice = None #Ligand lattice type
        self.llpar = None #Ligand lattice parameter
        self.apl = None   #Area per ligand
        self.xlpar = None #Crystal lattice parameter
    ...
    def add_ligands(self, ligands, pop_ratio, r0, balance_charge, lattice, llpar,
        ...
        llpar : float or None
            Ligand lattice parameter in nm^2. Ignored if `balance_charge` is True.
        ...
        #Ligand population
        self.lig_lattice = lattice
        if balance_charge:
            pop = self._ligpop_from_charge(ligands, pop_ratio, out_dir)
        else:
            self.llpar = llpar
            pop = self._ligpop_from_area(ligands, pop_ratio, out_dir)

        #Assign ligands to surfaces
        surfaces = self._ligands_to_surfaces(ligands, pop, r0)
        ...
        for surf in surfaces.values():
            ...
            #valign = np.array([stheta*cphi, stheta*sphi, 
            #                   math.copysign(ctheta, normal[2])])
            pop = surf['pop']
            ...
            for i, lg in enumerate(ligands):
                ...
                for mid in mids: 
                    ...
                    theta = math.radians(3)*rng.random()
                    phi = math.radians(3)*rng.random()
                    xi = math.radians(3)*rng.random()
                    stheta = math.sin(theta); ctheta = math.cos(theta)
                    sphi = math.sin(phi); cphi = math.cos(phi)
                    #valign = np.array([stheta*cphi, stheta*sphi, 
                    #                   math.copysign(ctheta, normal[2])])
                    valign = np.array([stheta*cphi, stheta*sphi, ctheta])
                    self.align(aid_head, aid_tail, axis=valign,
                               atm_ids=range(aid_beg, aid_end+1))

                    pivot = self.atoms[aid_head].coords
                    self.rotate(xi, valign, pivot,
                                atm_ids=range(aid_beg, aid_end+1))

                    if normal[2] < 0:
                        coords = self.get_atom_coords(atm_ids=range(aid_beg, aid_end+1))
                        coord_head = self.get_atom_coords([aid_head])
                        coords[:,2] -= coord_head[2]
                        coords[:,2] *= -1
                        coords[:,2] += coord_head[2]
                        self.set_atom_coords(range(aid_beg, aid_end+1), coords)

                    #Rotate randomly around the surface normal
                    #angle = 2*math.pi*rng.random()
                    #pivot = self.atoms[aid_head].coords
                    #self.rotate(xi, valign, pivot,
                    #            atm_ids=range(aid_beg, aid_end+1))
                    #add a small tilt (~10 deg) around [0,1,0]
                    #angle = math.radians(10+rng.random()-0.5)
                    #pivot = self.atoms[aid_head].coords
                    #self.rotate(angle, np.array([0,1,0]), pivot,
                    #            atm_ids=range(aid_beg, aid_end+1))

        #Create group for all ligands
        self.set_group('Ligands', molecule_ids=range(mid_beg, mid_end+1))
        #Create separate groups in case of mixed ligands
        if len(ligands) > 1:
            for each in ligands:
                name = each.name
                self.set_group(f"Ligands_{name}", molecule_names=[name])
    ...
    def _ligpop_from_area(self, ligands, pop_ratio, out_dir):
        """
        ...
        lo = [self.simbox[0,0], self.simbox[1,0]]
        hi = [self.simbox[0,1], self.simbox[1,1]]

        #Number of unit cells along x & y
        nucx = math.floor((hi[0]-lo[0])/self.llpar)
        nucy = math.floor((hi[1]-lo[1])/self.llpar)

        #Number of ligands
        if self.is_slab:
            area_tot = (hi[0]-lo[0])*(hi[1]-lo[1])
            if self.lig_lattice == 'sc':
                num_ligands = nucx*nucy 
                self.apl = self.llpar**2
            elif self.lig_lattice == 'bcc':
                num_ligands = 2*nucx*nucy
                self.apl = self.llpar**2/2
        ...
        pop = [round(num_ligands*f) for f in pop_frac[0:-1]]
        pop.append(num_ligands - sum(pop))
        #self.apl = area_tot/sum(pop)
        print(f"Area/ligand = {self.apl:f} nm^2\n"
              f"Brush density = {1/self.apl:f}/nm^2\n"
              f"Ligand lattice parameter = {self.llpar:f} nm", 
              file=open(f"{out_dir}/info.txt", 'a'))
        return pop


    def _ligpop_from_charge(self, ligands, pop_ratio, out_dir):
        """
    ...
    def _get_graft_points(self, n, lo, hi, boundary):
        """
        ...
        #apl = area/n (TODO: Change for each surface of use self.apl?)
        #if lattice == 'sc':
        #    lpar = math.sqrt(self.apl)
        #elif lattice == 'bcc':
        #    lpar = math.sqrt(2*self.apl)

        sites = get_lattice_points(self.lig_lattice, self.llpar, lo, hi, 
                                   boundary=boundary, num_sites=0)
        graft_points = rng.choice(sites, size=n, replace=False)
        return graft_points


    def _ligands_to_surfaces(self, ligands, pop, r0):
        """
        ...
        if self.is_slab:
            if self.slab_pos == 'bot':
                ...
                gp = self._get_graft_points(sum(pop), lo, hi, 'ppp')
                ...
                n = sum(pop_top)
                gp = self._get_graft_points(n, lo, hi, 'ppp')
                ...
                #pop_bot = [x-y for x,y in zip(pop,pop_top)] #May lead to
                #more ligands than the number of sites (integer division)
                pop_bot = [x//2 for x in pop]
                n = sum(pop_bot)
                gp = self._get_graft_points(n, lo, hi, 'ppp')
            ...
            n = sum(pop_left)
            gp = self._get_graft_points(n, lo, hi, 'nnn')
        ...
        if xtal_offset > 0:
            if self.is_slab:
                if self.slab_pos == 'bot':
                    ...
                    self.simbox[2,0] -= (xtal_offset+2)
                    self.simbox[2,1] += (xtal_offset+2)
            else:
